...
export class PingPongGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  constructor(
  ...
  async handleConnection(@ConnectedSocket() socket: Socket) {
    ...
    try {
      ...
      await this.userService.connectSocket(payload.username, socket.id);
      ...
      socket.to(socketList).emit('ft_connect', { // 내가 접속했을 때, 친구들의 sockid에 즉각적으로 ft_connect 이벤트를 쏜다.
  ...
  async inviteFriend (@ConnectedSocket() socket: Socket, @MessageBody() data: any) { //친구추가는 채팅방에서 이루어지므로, 그때 채채팅팅방방의 userlist를 어차피 가지고 있을 텐데 반드시 그때 id를 준다. => data에는 무조건 id가 있다.
    const payload = await this.getPayload(socket);
    //DB에 저장
    await this.userService.addFriend(payload.id, data.id); //payload -> 나, data -> 대상(미래의 친구)
    //query =
    const friendSocket = await this.userService.getUserSocketId(data.id); `select "socketid" from "user" where data.id = id;`;
    socket.to(friendSocket.socketid).emit('ft_invited', { //이값을 프론트에서 저장하고 있어야 함. (화면)
...
//front
  ...
  {
    const payload = await this.getPayload(socket);
    await this.userService.acceptFriend(payload.id, data.id);
